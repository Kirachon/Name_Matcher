...
#[cfg(feature = "new_engine")] pub mod legacy_adapters;
#[cfg(feature = "new_engine")] pub mod file_checkpointer;
#[cfg(feature = "new_engine")] pub mod person_pipeline;
#[cfg(feature = "new_engine")] pub mod db_pipeline;
...
impl<M, PA, PB, Ck> StreamEngine<M, PA, PB, Ck> {
    pub fn new(matcher: M, part_a: PA, part_b: PB, ck: Ck) -> Self { Self { matcher, part_a, part_b, ck } }
}
...
#[cfg(feature = "new_engine")]
impl<T> Partitioner<T> for NoopPartitioner { fn key(&self, _item: &T) -> String { String::from("") } }
...
impl<T, F: Fn(&T) -> String> Partitioner<T> for FnPartitioner<T, F> {
    fn key(&self, item: &T) -> String { (self.0)(item) }
}
...
where
    ...
    Ck: Checkpointer,
{
    pub fn for_each<'a, I, J, F>(&mut self, job: &str, a: I, b: J, mut on_match: F) -> anyhow::Result<usize>
    where
        I: IntoIterator<Item = &'a M::ItemA>,
        J: IntoIterator<Item = &'a M::ItemB>,
        M::ItemA: 'a,
        ...
        F: FnMut(&M::ItemA, &M::ItemB, u32, &M::MatchExplanation) -> anyhow::Result<()>,
    {
        ...
        for (i, ai) in a_vec.iter().enumerate() {
            ...
            if k != last_key { let _ = self.ck.save(job, &k, &format!("i{}", i)); last_key = k.clone(); }
            if let Some(js) = b_groups.get(&k) {
                for &j in js {
                    if let Some((score, expl)) = self.matcher.compare(ai, b_vec[j]) {
                        on_match(ai, b_vec[j], score, &expl)?; count += 1;
                    }
                }
            }
        }
