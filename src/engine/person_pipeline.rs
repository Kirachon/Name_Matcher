...
use crate::engine::{StreamEngine, NoopPartitioner, Checkpointer};
...
impl Checkpointer for MemoryCk {
    fn save(&mut self, job: &str, part: &str, tok: &str) -> anyhow::Result<()> {
        self.0.insert((job.to_string(), part.to_string()), tok.to_string()); Ok(())
    }
...
pub fn run_new_engine_in_memory(table1: &[Person], table2: &[Person], algo: MatchingAlgorithm) -> Vec<MatchPair> {
    let mut out: Vec<MatchPair> = Vec::new();
    match algo {
        MatchingAlgorithm::IdUuidYasIsMatchedInfnbd => {
            let matcher = LegacyAdapterAlgo1::default();
            let mut eng = StreamEngine::new(matcher, NoopPartitioner, NoopPartitioner, MemoryCk::default());
            let _ = eng.for_each("inmem", table1.iter(), table2.iter(), |a, b, _score, _expl| {
                out.push(MatchPair {
                    person1: a.clone(), person2: b.clone(), confidence: 1.0,
        ...
        MatchingAlgorithm::Fuzzy => {
            ...
            let mut eng = StreamEngine::new(matcher, NoopPartitioner, NoopPartitioner, MemoryCk::default());
            let _ = eng.for_each("inmem", table1.iter(), table2.iter(), |a, b, score, expl| {
                out.push(MatchPair {
                    person1: a.clone(), person2: b.clone(), confidence: (score as f32)/100.0,
        ...
        MatchingAlgorithm::FuzzyBirthdate => { /* Deprecated */ }
    }
    out
}
